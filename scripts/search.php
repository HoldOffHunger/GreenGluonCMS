			...
			if($text_pieces_count > 1) {
				...
				for($i = 0; $i < $search_term_piece_count; $i++) {
					...
					if($search_term_piece_text_pieces_count > 1) {
						...
						$earlyexit = 0;
						for($j = 2; $j < $search_term_piece_text_pieces_count; $j++) {
							if(strlen($search_term_text) < (2 * $string_length) && $search_term_piece_text_pieces[$j]) {
								$text_left_side = $search_term_piece_text_pieces[$j - 1];
								$text_right_side = $search_term_piece_text_pieces[$j];
								...
								$j = $search_term_piece_text_pieces_count;
								$earlyexit = 1;
							}
						}
