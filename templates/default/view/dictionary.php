<?php
	...
	$header_primary_args = [
		'title'=>'Dictionary of ' . $this->entry['Title'],
		'image'=>$primary_image,
	...
	/*
	print("<PRE>");
	print_r($this->entrydictionary);
	print("</PRE>");
	*/
	
	$definitions_count = count($this->entrydictionary);
	
	if($definitions_count)
	{
		foreach($this->entrydictionary as $word => $definition_objects)
		{
				// View Word
			...
			foreach($definition_objects as $definition_object)
			{
				$definition = $definition_object['Definition'];
				$author = $definition_object['Author'];
				$author_code = $definition_object['AuthorCode'];
				$source = $definition_object['Source'];
				$source_perma_id = $definition_object['SourcePermaid'];
				$publication_date = $definition_object['PublicationDate'];
				print('<li>');
				print($definition);
				print(' (');
				print('<a href="/people/' . $author_code . '/view.php">' . $author . '</a>, ');
				print('from <a href="/?id=' . $source_perma_id . '">' . $source . '</a>');
				if($publication_date) {
					print(', ');
