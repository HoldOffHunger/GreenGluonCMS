<?php
	...
	$header_primary_args = array(
		'indentlevel'=>1,
		'title'=>$this->domain_object->primary_domain . ' System Status : View All Lists and Items',
	...
	$version_list_display_args = array(
		...
		'list'=>[
			[
				'<nobr>Other List Actions:</nobr>', ' &bull; <a href="dbstatus.php?action=CreateList">Create List</a><br> &bull; <a href="dbstatus.php?action=ViewAllLists">View All Lists</a>',
			],
		],
	);
	...
	$loaded_lookup_lists = $this->loaded_lookup_lists;
	
	foreach($loaded_lookup_lists as $loaded_lookup_list)
	{
		$divider->displaystart($divider_padding_start_args);
		...
		$version_list_display_args = [
			...
			'list'=>[
				[
					'<nobr>' . 'LookupList' . $loaded_lookup_list['id'] . ' : ' .'</nobr>', $loaded_lookup_list['Title'],
				],
			],
		];
		$generic_list->Display($version_list_display_args);
		...
		if($loaded_lookup_list[itemsdisplay] && count($loaded_lookup_list[itemsdisplay]))
		{
			$items_list = $loaded_lookup_list['itemsdisplay'];
		}
		else
		{
			$items_list = [['None.']];
		...
		$version_list_display_args = array(
			'options'=>array(
				'indentlevel'=>1,
				'tableheaders'=>0,
				'tableclass'=>'width-90percent horizontal-center border-2px background-color-gray13',
				'rowclass'=>'border-1px horizontal-left',
				'cellclass'=>array(
					'border-1px vertical-top',
					'border-1px width-100percent vertical-top',
