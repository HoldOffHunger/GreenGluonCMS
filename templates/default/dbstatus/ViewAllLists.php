<?php
	...
	$version_list_display_args = array(
		...
		'list'=>[
			[
				'<nobr>Other List Actions:</nobr>', ' &bull; <a href="dbstatus.php?action=CreateList">Create List</a><br> &bull; <a href="dbstatus.php?action=ViewAllListsAndItems">View All Lists and Items</a>',
			],
		],
	);
	...
	$loaded_lookup_lists = $this->lookup_lists;
	...
	foreach($loaded_lookup_lists as $loaded_lookup_list)
	{
		...
		$version_list_display_args = [
			...
			'list'=>[
				[
					'<nobr>' . 'LookupList' . $loaded_lookup_list['id'] . ' : ' .'</nobr>',
					'<a href="dbstatus.cgi?action=EditList&LookupListid=' . $loaded_lookup_list['id'] . '" target="_blank">' . $loaded_lookup_list['Title'] . '</a>',
					'<nobr>' . $loaded_lookup_list['itemscount'] . ' items</nobr>',
				],
			],
		];
		$generic_list->Display($version_list_display_args);
