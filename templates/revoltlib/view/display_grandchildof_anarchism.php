<?php
	...
	if($image_count < 1) {
		$get_record_where = [
			type=>'Image',
			'definition'=>[
				'Entryid'=>$this->parent['id'],
			],
		];
		$images = $this->db_access_object->GetRecords($get_record_where);
		
		$this->entry['image'] = $images;
		$image_count = count($this->entry['image']);
	}
	...
	elseif ($this->script_format_lower == 'html')
	{
		...
		$header_primary_args = [
			'title'=>$this->parent['Title'] . ' : ' . $this->header_title_text,
			'image'=>$primary_image,
...
#		print_r($this->entry['association']);
		print("\n\n");
#		print_r($this->parent['association']);
		print("-->");
		...
		if($this->entry['association'] && $association_count)
		{
			...
			for($i = 0; $i < $association_count; $i++)
			{
				...
				$child_title .= $association['SubType'];
				$child_title .= ' : ';
